...
export default function MarkdownViewer({ nodeId }: MarkdownViewerProps) {
  ...
  const htmlContent = useMemo(() => {
    ...
    try {
      // Create a customized marked renderer to securely handle external links
      const renderer = new marked.Renderer();
      const originalLink = renderer.link.bind(renderer);

      renderer.link = (href, title, text) => {
        // Call the original renderer first to ensure inputs are HTML-escaped safely
        ...
        if (
          ...
          // Inject secure target and rel attributes for external links
